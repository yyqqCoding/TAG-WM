...
if __name__ == '__main__':
    ...
    def process_images(input_path, img_output_path, mask_output_path, logo_putting_nums, logo_putting_ratios, second_name):
        ...
        num = 0
        for logo_putting_num, logo_putting_ratio in zip(logo_putting_nums, logo_putting_ratios):
            # if second_name == 'DDIM' and logo_putting_num == 1 and logo_putting_ratio == 0.7:
            #     continue
            args.logo_putting_num, args.logo_ratio = logo_putting_num, logo_putting_ratio
            logo_instance = Logo(args)
            
            img_path = os.path.join(img_output_path, f'num{str(logo_putting_num)}_ratio{str(logo_putting_ratio)}')
            mask_path = os.path.join(mask_output_path, f'num{str(logo_putting_num)}_ratio{str(logo_putting_ratio)}')
            ...
            for image_name in os.listdir(input_path):
                ...
                output_image_path = os.path.join(img_path, image_name)
                output_mask_path = os.path.join(mask_path, image_name)
